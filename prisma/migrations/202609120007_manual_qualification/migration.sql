...
BEGIN
 ...
 IF p_id IS NULL THEN
   IF EXISTS(SELECT 1 FROM public."Lead" l WHERE l."organizationId"=org AND (
      (p_data->>'email' IS NOT NULL AND lower(l.email)=p_data->>'email') OR
   ...
   INSERT INTO public."Lead" ("organizationId",id,name,email,phone,source,"brokerId","dataQuality","humanRequired","provisionalContactBlock","suppressionReason",version,"updatedAt")
   VALUES(org,lead_id,p_data->>'name',p_data->>'email',p_data->'phones'->0->>'number','manual',CASE WHEN role_name='BROKER' THEN broker ELSE NULL END,
     jsonb_build_object('phones',p_data->'phones','analysisPending',true),true,true,'Cadastro manual: revisar contexto e permissão de contato',lead_version,now());
 ELSE
 ...
 IF p_id IS NULL THEN
   ...
   event_payload:=jsonb_build_object('name',p_data->'name','email',p_data->'email','phone',p_data->'phones'->0->'number','operationType',p_data->'operationType','profile',profile,'originalCreatedAt',NULL);
 ELSE
